              ...
              Api.wxRequest(lotteryApi,'GET',{}, (res)=>{
                if (res.data.status*1===200) {
                  ...
                  if (res.data.data.prize.name==='谢谢参与') {
                    ...
                    clearTimeout(that.data.timer)   // 清除转动定时器，停止转动
                    that.setData({
                      pindex: that.data.prizeList.length-1,nextTips: true, nextTips: true, prize: -1,times: 0,click: true
                    });
                  } else {
                  ...
                  setTimeout(()=>{
                    clearTimeout(that.data.timer);   // 清除转动定时器，停止转动
                    that.setData({
                      pindex: that.data.prizeList.length-1, nextTips: true, prize: -1,times: 0,click: true
                    });
